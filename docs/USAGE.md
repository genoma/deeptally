...
3. On success the popover prints *"Imported the key from your zsh login shell."* and refreshes the balance
   immediately.
...
| `API key: Keychain unreadable` | An item exists but could not be read (locked keychain, policy). |
...
converted (amounts below are illustrative):
...
`Resources/PriceTable.json`, which the app and the CLI share, and which you can override per user — see
[`ARCHITECTURE.md`](ARCHITECTURE.md).

`deeptally key import` is the only command that runs a shell. `deeptally key status` reports the source and
the key's shape (`source: keychain`, `shape: <n> chars, starts with sk-`) and never the key itself — the
length is the only number it will tell you, and only the constant `sk-` prefix is ever echoed.
`deeptally usage` is still the Step 3 stub: it prints `Usage summary lands in Step 4 (see docs/PLAN.md).`,
or `{"status":"not_implemented","step":4}` with `--json`.

**Exit codes** are contractual for scripts: `0` success · `2` no usable key · `1` a usage error or any other
...
| *"Could not read the API key from the Keychain (…)."* | An item exists but the read failed; the parenthesis carries a Keychain status, never a secret. |
| *"Pricing data problem: … No prices are shown until it is fixed."* | `PriceTable.json` is missing or invalid — the rate panel is suppressed rather than guessing. |
...
- A **USD per 1M tokens** table — Model / Cache-hit / Cache-miss / Output — with the effective price of each
  model for the window that is in force. Two decimals, and up to two more for three-decimal cache-hit rates.

The classification is computed in UTC (peak is 01:00–04:00 and 06:00–10:00 UTC, Monday–Friday, excluding
Chinese public holidays); only the presentation is local. If the price table cannot be read, this section
says *"Rate information is not available yet."* and a banner above names the problem.
...
The settings block — refresh cadence, threshold, menu bar metric, notifications, currency and the key row.
It sits below the popover's fold (scroll the popover to reach it), and it is all in the reference below.
...
| **Notify on low balance** | switch | on | A macOS notification when the balance falls below the threshold: *"DeepSeek balance is low"* / *"Balance $1.42 is below your $2.00 threshold."* macOS asks for permission once, at launch. |
| **Notify again after** | stepper | 12h | 15 min – 7 days (10 080 min) in 30-minute steps, shown as `12h`, `30 min`, `1h 30m`. A balance that stays low re-alerts at most once per cooldown, and the last alert time survives a relaunch. Only a low balance triggers an alert; an old reading neither triggers nor silences one. |
| **Currency** | text field | empty (`as-is`) | Intended as a display-currency code; empty means "show the account currency exactly as the API reports it", which is what the app does today. A non-empty code is stored and validated but is not yet applied to the amount — nothing converts currencies here, by decision ([`PLAN.md`](PLAN.md) §9). TODO: no step in [`PLAN.md`](PLAN.md) schedules wiring it up. |
...
| No low-balance notifications | macOS asks for permission once, at launch, when the switch is on. If it was denied, the switch keeps its state and the app keeps working: the popover's orange amount and *"Low balance — top up to keep requests running."* are the fallback. Re-allow DeepTally in **System Settings → Notifications**. |
| *"Pricing data problem: …"* / *"Holiday data problem: …"* | The price table (`Resources/PriceTable.json`, or your override at `~/.config/deeptally/PriceTable.json`) or the holiday list (`Resources/ChinaHolidays.json`) could not be read. Prices are data, so the app suppresses the rate panel instead of inventing numbers. The banner text names the offending file. |
