...
|---|---|---|---|
| **Refresh every** | stepper | 30 min | 5–240 minutes between backstop refreshes. Opening the popover fetches when the reading is over a minute old, so the number you look at is current regardless of this setting. The wait doubles after each consecutive failure, capped at 1 hour, and an offset drawn once per install (up to 60 s) is added so installs do not poll in lockstep. On battery or in Low Power Mode the backstop is widened to at least an hour. Wake, display wake, a returning session or network, and a power change check the balance if it is over 5 minutes old; a failed attempt always retries. |
...
| Balance looks stale | A reading older than twice the refresh interval gets the line *"3h 12m old"* and the status *"Balance is not up to date."* DeepTally keeps showing it instead of an empty panel. Open the popover: looking at the panel refreshes a reading older than a minute. Or wait: after a failure the next attempt is at most 1 hour + 60 s away, and a wake, a returning network or a power change triggers a check immediately. DeepSeek's balance is eventually consistent — the "as of" time is when it was fetched, not a claim about the server. |
