...
there. **The tap repository (`genoma/homebrew-tap`) is created with the first release** and updated per
release with the release's own `deeptally.rb` asset; a formula that points at a tag before the
...
Verify `SHA256SUMS` as described in [`UNSIGNED.md`](UNSIGNED.md), including the DMG-only form. The
Gatekeeper screenshots in `INSTALL.md` document the flow as measured on macOS 27 (2026-09-26); re-check
them on a new macOS release, and reproduce a quarantined launch with `make dmg SIMULATE=1`.
